...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # NOTE: We changed the the type of the self.corners attribute to a set
        #       to make unordered comparison easier. This is important since we
        #       want to make sure Pacman can visit the corners in any order he
        #       finds feasible.
...
def cornersHeuristic(state, problem):
    ...
    for corners in itertools.permutations(problemCorners):
        s = currentLowest + manhattanShortestPath(corners)
        if s < shortestPath:
            shortestPath = s
...
def manhattanShortestPath(unvisitedCorners):
    s = 0
    for i in range(len(unvisitedCorners) - 2):
